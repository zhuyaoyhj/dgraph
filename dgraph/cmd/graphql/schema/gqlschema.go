...
func preGQLValidation(schema *ast.SchemaDocument) gqlerror.List {
	...
	for _, defn := range schema.Definitions {
		errs = append(errs, applyDefnValidations(defn)...)
	}
...
func postGQLValidation(schema *ast.Schema) gqlerror.List {
	...
	for _, typ := range schema.Types {
		errs = append(errs, applyTypeValidations(typ)...)
...
func applyDefnValidations(defn *ast.Definition) gqlerror.List {
	var errs []*gqlerror.Error

	for _, rule := range defnValidations {
		if err := rule(defn); err != nil {
...
func applyTypeValidations(defn *ast.Definition) gqlerror.List {
	var errs []*gqlerror.Error
